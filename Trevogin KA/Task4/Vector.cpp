...
void CutTheMatrix(double** mas, double** p, int i, int j, int m)
{
...
double Det(double** mas, int m,double *dres,int ii,bool three) {
	int  j, k, n;
	double d;
	double** p;
	p = new double* [m];
	...
	if (m > 2) {
		for (int i = 0; i < m; i++) {
			CutTheMatrix(mas, p, 0, i, m);
			d = d + (k * mas[0][i] * Det(p, n,dres,i, three));
			if (three == false)
			dres[ii] = d;
			k = -k;
		}
...
Vector Vector::vectorProizv(const Vector* V)
{
	bool _3;
	if (Size == 3)
		_3 = true;
	...
	CutTheMatrix(m, p, 0, 0, Size);
	 Det(p, Size-1, mRes, 0,_3);

	 d = mRes[0];
	Det(m, Size, mRes, 0,_3);
	for (int i = 1; i < Size; i++)
	{
		res.Data[0] = d;
		res.Data[i] = mRes[i] * pow(-1,i);
	}
...
double Vector::lenghtbwnvec(const Vector& tmp)
{
	double res = 0;
	if (this->Data != nullptr && this->Size > 0)
	{
...
std::istream& operator>>(std::istream& in, const Vector& tmp)
{
	for (int i = 0; i < tmp.Size; i++)
	{
		cout << "Input "<<i+1<<" coordinate"<<endl;
		in >> tmp.Data[i];
	}
	return in;
}
